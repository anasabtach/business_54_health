@if( !empty($data) )
    @foreach( $data as $record )
        <div class="col-12 col-sm-6 col-md-4 col-lg-3 pt-10">
            <div class="deal-card">
            <a href="{{ route('vendor-detail',['name' => $record->slug ]) }}">
                ...
                <div class="card-text">
                    <ul class="review-star d-flex align-items-center">
                        ...
                        <li class="ms-2 color-2e3336">({{ $record->total_review > 1 ? "{$record->total_review} reviews" : "{$record->total_review} reviews" }})</li>
                    </ul>
                    <div class="card-title">
                        <p class="color-2e3336 font-18">{{ Str::limit($record->name,25,' ...') }}</p>
                        <p class="color-333 font-14">{{ Str::limit($record->about, 100,' ...') }}</p>
                    </div>
                </div>
